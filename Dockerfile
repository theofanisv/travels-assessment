...
COPY --from=sail start-container /usr/local/bin/start-container
COPY --from=sail supervisord.conf /etc/supervisor/conf.d/supervisord.conf
COPY --from=sail php.ini /etc/php/8.3/cli/conf.d/99-sail.ini
